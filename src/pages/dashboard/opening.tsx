...
const Opening: FC<OpeningProps> = ({ days }) => {
	...
	const initializeOpeningHrs = (days: Day[]) => {
		return days.map((day, index) => {
			const name = weekdayIndexToName(index)
			return {
				name,
				openTime: day ? day.openTime : undefined,
				closeTime: day ? day.closeTime : undefined,
			}
		})
	}
	const [openingHrs, setOpeningHrs] = useState(initializeOpeningHrs(days))
	...
	function _changeTime(day: Day) {
		return function (time: string, type: 'openTime' | 'closeTime') {
			const dayName = weekdayIndexToName(day.dayOfWeek)
			const index = openingHrs.findIndex((x) => x.name === dayName)
			if (index !== -1) {
